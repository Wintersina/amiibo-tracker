.PHONY: help test test-watch test-coverage lint format clean build run stop logs shell scrape scrape-remote deploy report-daily report-daily-dry report-daily-remote install-certs update-amiibo-db update-amiibo-db-dry seed-prices seed-prices-dry seed-prices-local seed-prices-remote amiibo-stats amiibo-stats-token amiibo-stats-remote owners
...
help:
	...
	@echo "  (override SITE_URL=https://staging.example.com)"
	@echo ""
	@echo "Amiibo collection stats:"
	@echo "  make amiibo-stats         - Most collected / rarest / owned by nobody"
	@echo "  make amiibo-stats-remote  - Same report, generated by the deployed app"
	@echo "  make amiibo-stats-token   - Print the token /api/amiibo-stats/ expects"
	@echo "  make owners               - Show/set who may read the stats API"
	@echo "  (TOP=25, AMIIBO=Mario to look up one, CSV=stats.csv for the full table)"
	@echo ""
...
# Tracked-amiibo stats
#
# Reports on what users actually collect, from the `amiibos` blob on each
# user_interactions document. Reads Firestore, so it needs GCP credentials
# (gcloud auth application-default login) — or just use amiibo-stats-remote.
#
# Usage:
#   make amiibo-stats                    # overview, top 10
#   make amiibo-stats TOP=25
#   make amiibo-stats AMIIBO=Mario       # holders of one amiibo (id or name)
#   make amiibo-stats CSV=stats.csv      # full per-amiibo table
#   make amiibo-stats-token              # token the HTTP endpoint expects
#   make amiibo-stats-remote             # hit the deployed endpoint instead
amiibo-stats:
	@echo "🎯 Building amiibo collection stats..."
	@$(ENSURE_VENV) \
	$(ENV_LOAD) \
	$(PYTHON) manage.py amiibo_stats \
		$(if $(TOP),--top $(TOP),) \
		$(if $(AMIIBO),--amiibo "$(AMIIBO)",) \
		$(if $(CSV),--csv $(CSV),)

# Prints the shared secret /api/amiibo-stats/ expects. The token is derived from
# DJANGO_SECRET_KEY, so which key is in scope decides which token you get:
#
#   make amiibo-stats-token            # your .env key -> local/dev token
#   make amiibo-stats-token PROD=1     # production's key, read from tfvars
#   make amiibo-stats-token SECRET_KEY=...   # an explicit key
#
# PROD/SECRET_KEY are exported *after* the .env load on purpose — .env would
# otherwise overwrite them and print a token that silently doesn't work.
TFVARS ?= terraform/terraform.tfvars
PROD_SECRET_KEY = $(shell sed -n 's/^[[:space:]]*django_secret_key[[:space:]]*=[[:space:]]*"\(.*\)"[[:space:]]*$$/\1/p' $(TFVARS) 2>/dev/null)

# Resolves DJANGO_SECRET_KEY for the stats token. Shared by both targets below.
STATS_KEY_OVERRIDE = \
	$(if $(PROD),if [ -z "$(PROD_SECRET_KEY)" ]; then \
		echo "❌ Could not read django_secret_key from $(TFVARS)" >&2; exit 1; \
	fi; export DJANGO_SECRET_KEY="$(PROD_SECRET_KEY)";) \
	$(if $(SECRET_KEY),export DJANGO_SECRET_KEY="$(SECRET_KEY)";)

amiibo-stats-token:
	@$(ENSURE_VENV) \
	$(ENV_LOAD) \
	$(STATS_KEY_OVERRIDE) \
	$(PYTHON) manage.py amiibo_stats --print-token

# Who can read the stats API from a logged-in browser session. Stored in
# Firestore (not in this public repo), so it needs GCP credentials.
#
#   make owners                                  # show the current list
#   make owners EMAILS="a@x.com b@x.com"         # replace the list
owners:
	@$(ENSURE_VENV) \
	$(ENV_LOAD) \
	$(PYTHON) manage.py set_owners $(EMAILS)
...
# Generates the amiibo stats report on the deployed app and prints it here.
# The token is taken from AMIIBO_STATS_TOKEN if set, otherwise derived from the
# DJANGO_SECRET_KEY in your environment — which must be production's for the
# remote call to authenticate. Sent as a header so it stays out of access logs.
#
#   make amiibo-stats-remote
#   make amiibo-stats-remote TOP=25
#   make amiibo-stats-remote AMIIBO=Mario
#   make amiibo-stats-remote CSV=stats.csv
amiibo-stats-remote:
	@echo "🎯 Fetching amiibo stats from $(SITE_URL)/api/amiibo-stats/"
	@$(ENSURE_VENV) \
	$(ENV_LOAD) \
	$(STATS_KEY_OVERRIDE) \
	TOKEN="$${AMIIBO_STATS_TOKEN:-$$($(PYTHON) manage.py amiibo_stats --print-token 2>/dev/null)}"; \
	if [ -z "$$TOKEN" ]; then \
		echo "❌ No token available. Set AMIIBO_STATS_TOKEN, or put production's"; \
		echo "   DJANGO_SECRET_KEY in .env (see terraform.tfvars: django_secret_key)."; \
		exit 1; \
	fi; \
	curl -sS -G -w "\nHTTP %{http_code}\n" \
		-H "X-Stats-Token: $$TOKEN" \
		--data-urlencode "top=$(if $(TOP),$(TOP),10)" \
		$(if $(AMIIBO),--data-urlencode "amiibo=$(AMIIBO)",) \
		$(if $(CSV),--data-urlencode "format=csv" -o $(CSV),) \
		$(SITE_URL)/api/amiibo-stats/
	@$(if $(CSV),echo "✅ Wrote $(CSV)",)
