...
class AmiiboLifeScraper(LoggingMixin):
    """
    Scraper for amiibo.life releases page.
    ...
    def scrape_amiibo_life(self):
        ...
        try:
            ...
            # Find all figure cards (skip game cards)
            figure_cards = soup.find_all("div", class_="figure-card")
            ...
            for card in figure_cards:
                try:
                    ...
                    # Extract name from <p class="name">
                    name_tag = card.find("p", class_="name")
                    if not name_tag:
                        continue
                    name = name_tag.get_text(strip=True)

                    # Extract series info from <p class="series">
                    series_tag = card.find("p", class_="series")
                    series_text = ""
                    amiibo_type = "Figure"
                    ...
                    # Extract release dates from the same row
                    parent_tr = card.find_parent("tr")
                    release_dates = {}

                    if parent_tr:
                        dates_cell = parent_tr.find("td", class_="release_dates_cell")
                        if dates_cell:
                            time_tags = dates_cell.find_all("time")
                                    ...
                                    if region_img:
                                        ...
                                        for key, code in region_map.items():
                                            if key in region_name:
                                                release_dates[code] = date_str
                                                break

                    # Skip sets and bundles
                    ...
                    amiibos.append(
                        {
                            "name": name,
                            "series": self.clean_series(series_text),
            ...
            self.log_info(f"Scraped {len(amiibos)} amiibos from amiibo.life")
            return amiibos
    ...
    def find_best_match(self, scraped_amiibo, existing_amiibos):
        ...
        Args:
            scraped_amiibo: Dict with 'name', 'release_dates', etc.
        ...
        for amiibo in existing_amiibos:
            ...
            date_boost = 0
            if scraped_dates:
                existing_release = amiibo.get("release", {})

                # Check all regions for matches
                for region in ["na", "eu", "jp", "au"]:
                    scraped_date = scraped_dates.get(region)
                    existing_date = existing_release.get(region)

                    if scraped_date and existing_date:
                        if scraped_date == existing_date:
                            # Exact date match gives significant boost
                            date_boost = max(date_boost, 0.3)
                        elif self.dates_are_close(scraped_date, existing_date):
    ...
    def update_amiibo(self, existing_amiibo, scraped_data):
        ...
        if scraped_image and not existing_amiibo.get("image"):
            existing_amiibo["image"] = scraped_image
            updated = True
    ...
    def create_placeholder_amiibo(self, scraped_data):
        ...
        """
        release_dates = scraped_data.get("release_dates", {})
        ...
        return {
            ...
            "tail": unique_tail,
            "type": scraped_data.get("type", "Figure"),
            "is_upcoming": True,
        }
    ...
    def backfill_amiibo_data(self, placeholder, api_amiibo):
        ...
        # Only use API image if we don't have one from amiibo.life
        ...
        if api_release:
            if "release" not in placeholder:
                placeholder["release"] = {}
            # Keep existing dates from amiibo.life if present
            for region in ["jp", "eu", "au", "na"]:
...
# DEPRECATED: This scraper is no longer used in production
# Kept for reference and potential fallback
class NintendoDotComScraper(LoggingMixin):
    """
    DEPRECATED: Scraper for Nintendo.com amiibo lineup page.
    Use AmiiboLifeScraper instead for production.
    Cloud Run compatible - uses file timestamp for cache checking.
    """
